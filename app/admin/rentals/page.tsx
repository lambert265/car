  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <h1 className="text-white font-bold text-2xl">Rental Bookings</h1>
      ...
      <div className="bg-[#0d0d0d] border border-white/[0.06] overflow-hidden">
          ...
          <table className="w-full min-w-[800px]">
            <thead>
              <tr className="border-b border-white/[0.06]">
                {["Client","Vehicle","Pick-up","Return","Rate","Status","Actions"].map((h) => (
            ...
            <tbody className="divide-y divide-white/[0.04]">
              {loading ? Array.from({length:4}).map((_,i) => (
                ...
                <tr key={item.id} className="hover:bg-white/[0.02] transition-colors">
                  ...
                  <td className="px-5 py-4 text-white/40 text-[12px]">{item.pickup_date ? new Date(item.pickup_date).toLocaleDateString() : "—"}</td>
                  <td className="px-5 py-4 text-white/40 text-[12px]">{item.return_date ? new Date(item.return_date).toLocaleDateString() : "—"}</td>
                  <td className="px-5 py-4 text-[#C9A84C] font-bold text-[12px]">{item.daily_rate ? `$${item.daily_rate}/day` : "—"}</td>
                  <td className="px-5 py-4">
                  ...
                  <td className="px-5 py-4">
                    <div className="flex gap-1.5 flex-wrap">
                      {["confirmed","active","completed","cancelled"].filter(s => s !== item.status).map((s) => (
        ...
        {!loading && filtered.length === 0 && (
          <div className="text-center py-16 text-white/20 text-[13px]">No rental bookings found</div>
        )}
      </div>
    </div>
  );
